#generate points randomly as train data 
...
class NeuralNetwork:
    def __init__(self, input_size=2, hidden_layers=[3, 4, 3], output_size=5, learning_rate=0.01, epochs=1000):
...
# Generate training data with random points
def generate_train_data():
    ...
    out_of_circle_class=len(diameters)
    
    data_X = np.random.uniform(-5, 30, (2, num_samples))
    data_Y = np.full(num_samples, out_of_circle_class)  # Default class is 'out of circle'
    
    for i, (radius, center) in enumerate(zip(diameters, circle_centers)):
        distances = np.sqrt((data_X[0] - center[0])**2 + (data_X[1] - center[1])**2)
        mask_inside_circle = distances <= radius
        data_Y[mask_inside_circle] = i  # Prioritize circle close to list tail
...
def plot_decision_boundary(nn, X, Y, test_points = None, test_colors = None):
    ...
    if (test_points is not None) and (test_colors is not None):
        plt.scatter(test_points[0, :], test_points[1, :], c=test_colors, edgecolors='k', marker='o', s=100)
...
nn = NeuralNetwork(input_size=2, hidden_layers=[3, 4, 3], output_size=5, learning_rate=0.01, epochs=1000000)
...
X_test = np.array([
    ...
    [6,   6],   # Point inside circle  1 (radius 5)
]).T  # Shape (2, 4)
